/*
  Warnings:

  - You are about to drop the column `dimedicalBrand` on the `Sucursal` table. All the data in the column will be lost.
  - Added the required column `admin` to the `User` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Sucursal" DROP COLUMN "dimedicalBrand";

-- AlterTable
ALTER TABLE "User" ADD COLUMN     "admin" BOOLEAN NOT NULL;
